...
mod tests {
    ...
    fn completed_is_never_classified() {
        ...
        assert_eq!(
            classify(None, false, now, true, Duration::hours(24)),
            DueState::None
        );
    }
    ...
    fn overdue_takes_priority() {
        let now = at(1_800_000_000);
        assert_eq!(
            classify(Some(now), false, now, true, Duration::hours(24)),
            DueState::Overdue
        );
        assert_eq!(
            classify(
    ...
    fn due_soon_uses_configured_threshold() {
        ...
        assert_eq!(
            classify(in2h, false, now, true, Duration::hours(1)),
            DueState::Upcoming
        );
        assert_eq!(
            classify(in2h, false, now, true, Duration::hours(3)),
            DueState::DueSoon
        );
        assert_eq!(
            classify(in2h, false, now, false, Duration::hours(3)),
            DueState::Upcoming
        );
    }
}
